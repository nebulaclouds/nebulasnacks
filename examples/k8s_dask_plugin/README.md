...
Nebula can natively execute [Dask](https://www.dask.org/) jobs on a Kubernetes Cluster,
...
which is created specifically for the Nebula task and subsequently torn down upon completion.

In the Nebula Kubernetes environment, the cost is amortized due to faster pod creation compared to machines.
...
Nebulakit enables writing Dask code natively as a task,
with the `Dask()` config automatically configuring the Dask cluster.
The example provided in this section offers a hands-on tutorial for writing Dask Nebula tasks.
...
2. Interactive Dask capabilities are not available with Nebula Kubernetes Dask;
   instead, it is better suited for running adhoc and scheduled jobs.

## Install the plugin

Install `nebulakitplugins-dask` using `pip` in your environment.
...
#### Step 1: Deploy Dask plugin in the Nebula backend

Nebula Dask utilizes the [Dask Kubernetes operator](https://kubernetes.dask.org/en/latest/operator.html)
in conjunction with a custom-built
[Nebula Dask plugin](https://pkg.go.dev/github.com/nebulaclouds/nebulaplugins@v1.0.28/go/tasks/plugins/k8s/dask).
To leverage this functionality, you need to enable the backend plugin in your deployment.
You can follow the steps mentioned in the {ref}`nebula:deployment-plugin-setup-k8s` section
to enable the Nebula Dask plugin for your deployment.
...
1. In the absence of specified resources, the
   [platform resources](https://github.com/nebulaclouds/nebula/blob/1e3d515550cb338c2edb3919d79c6fa1f0da5a19/charts/nebula-core/values.yaml#L520-L531)
   will be used.
...
2. When employing task resources, those will be enforced across all segments of the Dask job.
   ...
   > from nebulakitplugins.dask import Dask
   >
   > @task(
   >   task_config=Dask(),
...
3. When resources are designated for individual components, they hold the highest precedence.
   ...
   > ```python
   > from nebulakit import Resources, task
   > from nebulakitplugins.dask import Dask, Scheduler, WorkerGroup
   >
   > @task(
   >   task_config=Dask(
   >       scheduler=Scheduler(
...
> ```python
> from nebulakit import Resources, task, workflow, Labels, Annotations
> from nebulakitplugins.dask import Dask
>
...
```
pynebula run --remote dask_example.py \
