...
To enable the plugin in the backend, refer to the instructions provided in the {std:ref}`nebula:deployment-plugin-setup-k8s` guide.
...
.. testcode:: ray-quickstart-1
    ...
    from nebulakitplugins.ray import RayJobConfig
...
### Create a Ray cluster managed by Nebula and run a Ray Job on the cluster

```{eval-rst}
.. testcode:: ray-quickstart-2
    import ray
    from nebulakit import task
    from nebulakitplugins.ray import RayJobConfig, WorkerNodeConfig, HeadNodeConfig
...
```
pynebula run --remote ray_example.py \
    ray_workflow --n 10
```
