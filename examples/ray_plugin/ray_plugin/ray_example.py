...
#   Consequently, running Ray jobs becomes problematic in Nebula because Kubernetes imposes resource quotas in every project-domain namespace,
...
custom_image = ImageSpec(
    name="ray-nebula-plugin",
    registry="ghcr.io/nebulaclouds",
    packages=["nebulakitplugins-ray"],
)
...
# :::
#
# The following imports are required to configure the Ray cluster in Nebula.
# You can load them on demand.
# %%
if custom_image.is_container():
    import ray
    from nebulakitplugins.ray import HeadNodeConfig, RayJobConfig, WorkerNodeConfig
...
# %% [markdown]
# Include both {py:class}`~nebulakitplugins.ray.HeadNodeConfig` and
# {py:class}`~nebulakitplugins.ray.WorkerNodeConfig` in {py:class}`~nebulakitplugins.ray.RayJobConfig`.
...
# during which Nebula generates a self-contained Ray cluster on your local environment.
# %%
...
# ````
#
# You must ensure that the ingress controller is [installed](https://docs.nebula.org/en/latest/deployment/gcp/manual.html#ingress).
