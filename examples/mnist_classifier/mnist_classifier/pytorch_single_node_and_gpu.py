...
#
# Training a model on a single node on one GPU is as trivial as writing any Nebula task and simply setting the GPU to `1`.
# As long as the Docker image is built correctly with the right version of the GPU drivers and the Nebula backend is
# provisioned to have GPU machines, Nebula will execute the task on a node that has GPU(s).
#
# Currently, Nebula does not provide any specific task type for PyTorch (though it is entirely possible to provide a task-type
...
#
# In this example, we'll see how we can create any PyTorch model, train it using Nebula and a specialized container. The following video will outline the basics of this process.
#
# ```{eval-rst}
# ..  youtube:: sJrERMVtxL4
# ```
...
from nebulakit.types.file import PythonPickledFile
...
#
# :::{note}
# Note the usage of `requests=Resources(gpu="1")`. This will force Nebula to allocate this task onto a machine with GPU(s).
...
def pytorch_mnist_task(hp: Hyperparameters) -> TrainingOutputs:
    ...
    # after training the model, we can simply save it to disk and return it from the Nebula task as a {py:class}`nebulakit.types.file.NebulaFile`
    # type, which is the ``PythonPickledFile``. ``PythonPickledFile`` is simply a decorator on the ``NebulaFile`` that records the format
...
#
# :::{figure} https://raw.githubusercontent.com/nebulaclouds/static-resources/main/nebulasnacks/tutorials/pytorch/single-node/wandb_graphs.png
# :alt: Wandb Graphs
# :class: with-shadow
#
# Wandb Graphs
# :::
#
# :::{figure} https://raw.githubusercontent.com/nebulaclouds/static-resources/main/nebulasnacks/tutorials/pytorch/single-node/wandb_table.png
# :alt: Wandb Table
# :class: with-shadow
#
# Wandb Table
