...
def train_mnist(rank: int, world_size: int, hp: Hyperparameters):
    ...
    if rank == 0:
        ...
        # after training the model, we can simply save it to disk and return it from the Nebula
        # task as a `nebulakit.types.file.NebulaFile` type, which is the `PythonPickledFile`.
        # `PythonPickledFile` is simply a decorator on the `NebulaFile` that records the format
...
#
# :::{note}
# Note the usage of `requests=Resources(gpu=WORLD_SIZE)`. This will force Nebula to allocate this task onto a
...
# ## Defining the `task`
#
# Next we define the nebula task that kicks off the distributed training process. Here we call the
