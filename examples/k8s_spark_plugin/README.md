...
Nebula has the capability to directly execute Spark jobs on a Kubernetes Cluster.
...
In the Nebula environment, this cost is spread out over time due to the swiftness of creating pods compared to a full machine.
...
With Nebulakit, you can compose PySpark code natively as a task.
...
:::{note}
This plugin has been rigorously tested at scale, successfully managing more than 100,000 Spark Jobs through Nebula at Lyft.
...
[multi-cluster mode](https://docs.nebula.org/en/latest/deployment/configuration/performance.html#multi-cluster-mode).
...
Managing Python dependencies can be challenging, but Nebula simplifies the process
...
3. Nebula takes care of all the management tasks.

Cons:

1. Short-running, bursty jobs may not be the best fit due to container overhead.
2. Interactive Spark capabilities are not available with Nebula Kubernetes Dask;
   instead, it is better suited for running adhoc and scheduled jobs.

## Implementation details

### Step 1: Deploy Spark plugin in the Nebula backend

Nebula Spark employs the Spark on K8s operator in conjunction with a bespoke
[Nebula Spark Plugin](https://pkg.go.dev/github.com/nebulaclouds/nebulaplugins@v0.5.25/go/tasks/plugins/k8s/spark).

This plugin serves as a backend component and necessitates activation within your deployment.
To enable it, follow the instructions outlined in the {ref}`nebula:deployment-plugin-setup-k8s` section.
...
Install `nebulakitplugins-spark` using `pip` in your environment.
...
:::{note}
To enable Nebula to build the Docker image for you using `ImageSpec`, install `nebulakitplugins-envd`.
:::
...
Furthermore, Nebula offers the capability to generate direct links to both the Spark driver logs and individual Spark executor logs.

These Spark-related features, including the Spark history server and Spark UI links, are seamlessly displayed on the Nebula Console.
Their availability is contingent upon the following configuration settings:

#### Configure the Spark history link within the UI

To access the Spark history UI link within the Nebula Console,
it's necessary to configure a variable in the Spark section of the Nebulaplugins configuration.
...
[this link](https://github.com/nebulaclouds/nebulaplugins/blob/master/go/tasks/plugins/k8s/spark/config.go).
...
It's important to note that Nebula does not perform automatic log separation. You can review the configuration structure
[here](https://github.com/nebulaclouds/nebulaplugins/blob/master/go/tasks/plugins/k8s/spark/config.go#L31-L36).
...
For more comprehensive information, please consult the [configuration structure](https://github.com/nebulaclouds/nebulaplugins/blob/c528bb88937b4732c9cb5537ed8ea6943ff4fb56/go/tasks/plugins/k8s/spark/config.go#L24-L29).

## Run the examples on the Nebula cluster

To run the provided examples on the Nebula cluster, use any of the following commands:

```
pynebula run --remote pyspark_pi.py my_spark
```

```
pynebula run --remote dataframe_passing.py \
