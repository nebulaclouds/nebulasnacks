...
from nebulakit.types.directory import TensorboardLogs
from nebulakit.types.file import PNGImageFile, PythonPickledFile
...
custom_image = ImageSpec(
    name="nebula-kfpytorch-plugin",
    packages=["torch", "torchvision", "nebulakitplugins-kfpytorch", "matplotlib", "tensorboardX"],
...
# :::
#
# The following imports are required to configure the PyTorch cluster in Nebula.
...
if custom_image.is_container():
    ...
    from nebulakitplugins.kfpytorch import PyTorch, Worker
...
# or by initializing the [CUDA parameters](https://github.com/nebulaclouds/nebulakit/blob/master/nebulakit/image_spec/image_spec.py#L34-L35)
...
# %% [markdown]
# To create a PyTorch task, add {py:class}`~nebulakitplugins.kfpytorch.PyTorch` config to the Nebula task.
# %%
@task(
...
def mnist_pytorch_job(hp: Hyperparameters) -> TrainingOutputs:
    log_dir = os.path.join(nebulakit.current_context().working_directory, "logs")
    ...
    train_loader = torch.utils.data.DataLoader(
        datasets.MNIST(
            os.path.join(nebulakit.current_context().working_directory, "data"),
            train=True,
            download=True,
    ...
    # Save the model
    model_file = os.path.join(nebulakit.current_context().working_directory, "mnist_cnn.pt")
...
def plot_accuracy(epoch_accuracies: typing.List[float]) -> PNGImageFile:
    ...
    plt.ylabel("accuracy")
    plt.xlabel("epoch")
    accuracy_plot = os.path.join(nebulakit.current_context().working_directory, "accuracy.png")
...
#
# ```
# Model: /tmp/nebula/20210110_214129/mock_remote/8421ae4d041f76488e245edf3f4360d5/my_model.h5, plot PNG: /tmp/nebula/20210110_214129/mock_remote/cf6a2cd9d3ded89ed814278a8fb3678c/accuracy.png, Tensorboard Log Dir: /tmp/nebula/20210110_214129/mock_remote/a4b04e58e21f26f08f81df24094d6446/
# ```
#
...
#
# ```
# tensorboard --logdir /tmp/nebula/20210110_214129/mock_remote/a4b04e58e21f26f08f81df24094d6446/
# ```
#
# When executing remotely on the Nebula-hosted environment, the workflow execution outputs can be retrieved.
...
#
# Nebula supports distributed training through [torch elastic](https://pytorch.org/docs/stable/elastic/run.html) using `torchrun`.
...
#
# The process involves adding {py:class}`~nebulakitplugins.kfpytorch.Elastic` configuration to the Nebula task.
#
# ```python
# from nebulakitplugins.kfpytorch import Elastic
#
# @task(
#   task_config=Elastic(
#     nnodes=1,
...
# ```
#
# This initializes four worker processes, whether executed locally or remotely within a Kubernetes pod within a Nebula cluster.
...
# [IgnoreOutputs exception](https://docs.nebula.org/projects/nebulakit/en/latest/generated/nebulakit.core.base_task.IgnoreOutputs.html)
