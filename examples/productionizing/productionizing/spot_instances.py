...
# This can be done by setting taints and tolerations using the [config](https://github.com/nebulaclouds/nebulaplugins/blob/60b94c688ef2b98aa53a9224b529ac672af04540/go/tasks/pluginmachinery/nebulak8s/config/config.go#L84-L92) available at `nebulaclouds/nebulaplugins` repo.
#
...
#
# If specified, the `interruptible flag` is added to the task definition and signals to the Nebula engine that it may be scheduled on machines that may be preempted, such as AWS spot instances. This is low-hanging fruit for any cost-savings initiative.
#
...
# %% [markdown]
# By setting this value, Nebula will schedule your task on an auto-scaling group (ASG) with only spot instances.
#
# :::{note}
# If you set `retries=n`, for instance, and the task gets preempted repeatedly, Nebula will retry on a preemptible/spot instance `n-1` times and for the last attempt will retry your task on a non-spot (regular) instance. Please note that tasks will only be retried if at least one retry is allowed using the `retries` parameter in the `task` decorator.
# :::
#
# ### Which Tasks Should Be Set To Interruptible?
#
# Most Nebula workloads should be good candidates for spot instances.
