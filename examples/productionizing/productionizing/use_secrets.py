...
# ```
#
# Nebula supports running a variety of tasks, from containers to SQL queries and
# service calls, and it provides a native Secret construct to request and access
# secrets.
#
# This example explains how you can access secrets in a Nebula Task. Nebula provides
...
# - Have access to a Nebula cluster, for e.g. with `nebulactl demo start` as
#   described {ref}`here <getting_started_nebula_cluster>`.
# :::
#
# The first step to using secrets in Nebula is to create one on the backend.
# By default, Nebula uses the K8s-native secrets manager, which we'll use in this
...
# the secret in the `nebulasnacks` project under the `development` domain, replace `<project>-<domain>`
# with `nebulasnacks-development`. This is because secrets need to be in the same namespace as the
# workflow execution.
# :::
#
# :::{important}
...
#
# Once you've defined a secret on the Nebula backend, `nebulakit` exposes a class
# called {py:class}`~nebulakit.Secret`s, which allows you to request a secret
...
import nebulakit
from nebulakit import Secret, task, workflow
from nebulakit.testing import SecretsManager
...
# {py:func}`@task <nebulakit.task>` decorator. The request tells Nebula to make
# the secret available to the task.
#
# The secret can then be accessed inside the task using the
# {py:class}`~nebulakit.ExecutionParameters` object, which is returned by
# invoking the {py:func}`nebulakit.current_context` function, as shown below.
#
# At runtime, nebulakit looks inside the task pod for an environment variable or
...
def secret_task() -> str:
    context = nebulakit.current_context()
...
#
# :::{note}
# - In case Nebula fails to access the secret, an error is raised.
...
#
# It is also possible to make Nebula mount the secret as a file or an environment
# variable.
#
...
def secret_file_task() -> Tuple[str, str]:
    secret_manager = nebulakit.current_context().secrets
...
# secret request as well. For example, for the {py:class}`~nebulakitplugins.sqlalchemy.SQLAlchemyTask`
# you need to:
#
# 1. Specify the `secret_requests` argument.
# 2. Configure the {py:class}`~nebulakitplugins.sqlalchemy.SQLAlchemyConfig` to
#    declare which secret maps onto which connection argument.
#
# ```python
# from nebulakit import kwtypes
# from nebulakitplugins.sqlalchemy import SQLAlchemyTask, SQLAlchemyConfig
#
#
# # define the secrets
# secrets = {
...
#
# 1. Nebula invokes a plugin to create the K8s object. This can be a Pod or a more complex CRD (e.g. Spark, PyTorch, etc.)
#
#    :::{note}
#    The plugin ensures that the labels and annotations are passed to any Pod that is spawned due to the creation of the CRD.
#    :::
#
# 2. Nebula applies labels and annotations that are referenced to all secrets the task is requesting access to. Note that secrets are not case sensitive.
#
# 3. Nebula sends a `POST` request to `ApiServer` to create the object.
#
# 4. Before persisting the Pod, `ApiServer` invokes all the registered Pod Webhooks and Nebula's Pod Webhook is called.
#
# 5. Using the labels and annotiations attached in **step 2**, Nebula Pod Webhook looks up globally mounted secrets for each of the requested secrets.
#
...
#
# Once the secret is injected into the task pod, Nebulakit can read it using the secret manager.
#
# The webhook is included in all overlays in the Nebulakit repo. The deployment file creates two things; a **Job** and a **Deployment**.
#
# 1. `nebula-pod-webhook-secrets` **Job**: This job runs `nebulapropeller webhook init-certs` command that issues self-signed CA Certificate as well as a derived TLS certificate and its private key. Ensure that the private key is in lower case, that is, `my_token` in contrast to `MY_TOKEN`. It stores them into a new secret `nebula-pod-webhook-secret`.
# 2. `nebula-pod-webhook` **Deployment**: This deployment creates the Webhook pod which creates a MutatingWebhookConfiguration on startup. This serves as the registration contract with the ApiServer to know about the Webhook before it starts serving traffic.
#
# ## Secret Discovery
#
# Nebula identifies secrets using a secret group and a secret key, which can
# be accessed by {py:func}`nebulakit.current_context` in the task function
# body, as shown in the code examples above.
#
# Nebulakit relies on the following environment variables to load secrets (defined [here](https://github.com/nebulaclouds/nebulakit/blob/9d313429c577a919ec0ad4cd397a5db356a1df0d/nebulakit/configuration/internal.py#L141-L159)). When running tasks and workflows locally you should make sure to store your secrets accordingly or to modify these:
#
# - `NEBULA_SECRETS_DEFAULT_DIR`: The directory Nebulakit searches for secret files. **Default:** `"/etc/secrets"`
# - `NEBULA_SECRETS_FILE_PREFIX`: a common file prefix for Nebula secrets. **Default:** `""`
# - `NEBULA_SECRETS_ENV_PREFIX`: a common env var prefix for Nebula secrets. **Default:** `"_FSEC_"`
#
# When running a workflow on a Nebula cluster, the configured secret manager will use the secret Group and Key to try and retrieve a secret.
...
#
# When a task requests a secret, Nebulapropeller will try to retrieve secrets in the following order:
#
# 1. Checking for global secrets, i.e. secrets mounted as files or environment variables on the `nebula-pod-webhook` pod
...
#
# - [K8s secrets](https://kubernetes.io/docs/concepts/configuration/secret/#creating-a-secret) (**default**): `nebula-pod-webhook` will try to look for a K8s secret named after the secret Group and retrieve the value for the secret Key.
# - [AWS Secret Manager](https://docs.aws.amazon.com/secretsmanager/latest/userguide/create_secret.html): `nebula-pod-webhook` will add the AWS Secret Manager sidecar container to a task Pod which will mount the secret.
# - [Vault Agent Injector](https://developer.hashicorp.com/vault/tutorials/getting-started/getting-started-first-secret#write-a-secret) : `nebula-pod-webhook` will annotate the task Pod with the respective Vault annotations that trigger an existing Vault Agent Injector to retrieve the specified secret Key from a vault path defined as secret Group.
#
# You can configure the additional secret manager by defining `secretManagerType` to be either 'K8s', 'AWS' or 'Vault' in
# the [core config](https://github.com/nebulaclouds/nebula/blob/master/kustomize/base/single_cluster/headless/config/propeller/core.yaml#L34) of the Nebulapropeller.
#
...
# (for example `nebulasnacks-development`). K8s secrets can be mounted as either files or injected as environment variables into the task pod,
...
# Vault secrets can only be mounted as files and will become available under `"/etc/nebula/secrets/SECRET_GROUP/SECRET_NAME"`.
#
# Vault comes with various secrets engines. Currently Nebula supports working with both version 1 and 2 of the `Key Vault engine <https://developer.hashicorp.com/vault/docs/secrets/kv>` as well as the `databases secrets engine <https://developer.hashicorp.com/vault/docs/secrets/databases>`.
...
# %% [markdown]
# The group parameter is used to specify the path to the secret in the Vault backend. For example, if you have a secret stored in Vault at `"secret/data/nebula/secret"` then the group parameter should be `"secret/data/nebula"`.
...
# When using the database secrets engine, the secret key itself is arbitrary but is required by Nebula to name and identify the secret file. It is arbitray because the database secrets engine returns always two keys, `username` and `password` and we need to retrieve a matching pair in one request.
#
# **Configuration**
#
# You can configure the Vault role under which Nebula will try to read the secret by setting webhook.vaultSecretManager.role (default: `"nebula"`).
...
# **Annotations**
#
# By default, `nebula-pod-webhook` injects following annotations to task pod:
#
...
#
# It is possible to add extra annotations or override the existing ones in Nebula either at the task level using pod annotations or at the installation level.
# If Nebula administrator wants to set up annotations for the entire system, they can utilize `webhook.vaultSecretManager.annotations` to accomplish this.
#
# ## Scaling the Webhook
#
# ### Vertical Scaling
