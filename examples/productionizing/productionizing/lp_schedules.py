...
# ```
#
# {ref}`nebula:divedeep-launchplans` can be set to run automatically on a schedule using the Nebula Native Scheduler.
# For workflows that depend on knowing the kick-off time, Nebula supports passing in the scheduled time (not the actual time, which may be a few seconds off) as an argument to the workflow.
#
...
# %%
from nebulakit import CronSchedule, LaunchPlan  # noqa: E402
...
# Specifying this argument means that Nebula will pass in the kick-off time of the
...
from nebulakit import FixedRate, LaunchPlan  # noqa: E402
...
#
# After initializing your launch plan, [activate the specific version of the launch plan](https://docs.nebula.org/projects/nebulactl/en/latest/gen/nebulactl_update_launchplan.html) so that the schedule runs.
#
# ```bash
# nebulactl update launchplan -p nebulaexamples -d development {{ name_of_lp }} --version <foo> --activate
# ```
...
#
# ```bash
# nebulactl get launchplan -p nebulasnacks -d development
# ```

# %% [markdown]
# ## Deactivating a Schedule
#
# You can [archive/deactivate the launch plan](https://docs.nebula.org/projects/nebulactl/en/latest/gen/nebulactl_update_launchplan.html) to deschedule any scheduled job associated with it.
#
# ```bash
# nebulactl update launchplan -p nebulaexamples -d development {{ name_of_lp }} --version <foo> --archive
# ```

# %% [markdown]
# ## Platform Configuration Changes For AWS Scheduler
#
# The Scheduling feature can be run using the Nebula native scheduler which comes with Nebula. If you intend to use the AWS scheduler then it requires additional infrastructure to run, so these will have to be created and configured. The following sections are only required if you use the AWS scheme for the scheduler. You can still run the Nebula native scheduler on AWS.
#
# ### Setting up Scheduled Workflows
#
# To run workflow executions based on user-specified schedules, you'll need to fill out the top-level `scheduler` portion of the nebulaadmin application configuration.
#
...
# %% [markdown]
# - **scheme**: in this case because AWS is the only cloud back-end supported for executing scheduled workflows, only `"aws"` is a valid value. By default, the no-op executor is used and in case of sandbox we use `"local"` scheme which uses the Nebula native scheduler.
