...
# :target: https://blog.nebula.org/map-tasks-in-nebula
# ```
#
...
from nebulakit import Resources, map_task, task, workflow
...
# %% [markdown]
# To repeat the execution of the `a_mappable_task` across a collection of inputs, use the {py:func}`~nebulakit:nebulakit.map_task` function from nebulakit.
...
# %% [markdown]
# When defining a map task, avoid calling other tasks in it. Nebula
# can't accurately register tasks that call other tasks.  While Nebula
...
# For example, you can configure the map task to run on [AWS Batch](https://docs.nebula.org/en/latest/deployment/plugin_setup/aws/batch.html#deployment-plugin-setup-aws-array),
