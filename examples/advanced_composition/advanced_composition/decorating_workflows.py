...
import nebulakit
from nebulakit import NebulaContextManager, task, workflow
...
def setup():
    ...
    external_service.initialize(id=nebulakit.current_context().execution_id)


@task
def teardown():
    print("finish external service")
    external_service.complete(id=nebulakit.current_context().execution_id)


# %% [markdown]
# As you can see, you can even use Nebulakit's current context to access the `execution_id` of the current workflow
# if you need to link Nebula with the external service so that you reference the same unique identifier in both the
# external service and Nebula.
#
# ## Workflow Decorator
#
...
def setup_teardown(fn=None, *, before, after):
    ...
    def wrapper(*args, **kwargs):
        # get the current nebula context to obtain access to the compilation state of the workflow DAG.
        ...
        # under the hood, nebulakit compiler defines and threads
...
# 2. The [create_node](https://github.com/nebulaclouds/nebulakit/blob/9e156bb0cf3d1441c7d1727729e8f9b4bbc3f168/nebulakit/core/node_creation.py#L18) function
#    to create nodes associated with the `before` and `after` tasks.
# 3. When `fn` is called, under the hood Nebulakit creates all the nodes associated with the workflow function body
...
# and to learn more about how to extend Nebula at a deeper level, for example creating custom types, custom tasks, or
# backend plugins, see {ref}`Extending Nebula <plugins_extend>`.
#
