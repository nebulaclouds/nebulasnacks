...
# [bug report](https://github.com/nebulaclouds/nebula/issues/new?assignees=&labels=bug%2Cuntriaged&projects=&template=bug_report.yaml&title=%5BBUG%5D+).
# ```
#
...
#
# {ref}`Static workflows <workflow>` are created at compile time when you call `pynebula run`,
# `pynebula register`, or `pynebula serialize`. This means that the workflow is static
...
# {ref}`Dynamic workflows <dynamic_workflows>`, on the other hand, are compiled
...
from nebulakit.experimental import eager
...
#
# 1. Materialize the output of nebula tasks and subworkflows so you can operate
...
# 2. Provide an alternative way of achieving concurrency in Nebula. Nebula has
...
# not compile any workflow at all. Instead, they use the {py:class}`~nebulakit.remote.remote.NebulaRemote`
...
# `@eager` functions to run on a remote Nebula cluster
...
# `elif` statement, which will be executed in a separate Nebula task.
...
# %%
from nebulakit.experimental import EagerException
...
#
# As with most Nebula constructs, you can execute eager workflows both locally
# and remotely.
#
# ### Local Execution
#
...
#
# ### Remote Nebula Cluster Execution
#
# Under the hood, `@eager` workflows use the {py:class}`~nebulakit.remote.remote.NebulaRemote`
# object to kick off task, static workflow, and eager workflow executions.
#
# In order to actually execute them on a Nebula cluster, you'll need to configure
# eager workflows with a `NebulaRemote` object and secrets configuration that
...
# [nebulactl](https://docs.nebula.org/projects/nebulactl/en/latest/#configuration)-compatible
# config file and `my_client_secret_group` and `my_client_secret_key` are the
# {ref}`secret group and key <secrets>` that you've configured for your Nebula
# cluster to authenticate via a client key.

# %% [markdown]
# ### Sandbox Nebula Cluster Execution
#
# When using a sandbox cluster started with `nebulactl demo start`, however, the
...
# %% [markdown]
# ```{important}
# When executing eager workflows on a remote Nebula cluster, it will execute the
# latest version of tasks, static workflows, and eager workflows that are on
# the `default_project` and `default_domain` as specified in the `NebulaRemote`
# object. This means that you need to pre-register all Nebula entities that are
...
#
# Assuming that your `nebulakit` code is configured correctly, you will need to
# register all of the task and subworkflows that are used with your eager
# workflow with `pynebula register`:
#
# ```{prompt} bash
# pynebula --config <path/to/config.yaml> register \
#  --project <project> \
#  --domain <domain> \
#  --image <image> \
...
# ```
#
# And then run it with `pynebula run`:
#
# ```{prompt} bash
# pynebula --config <path/to/config.yaml> run \
#  --project <project> \
#  --domain <domain> \
#  --image <image> \
...
# because eager workflows are actually nebula tasks under the hood, which means
# that `pynebula run` has no way of knowing what tasks and subworkflows are
# invoked inside of it.
# ```

# %% [markdown]
# ## Eager Workflows on Nebula Console
#
# Since eager workflows are an experimental feature, there is currently no
# first-class representation of them on Nebula Console, the UI for Nebula.
# When you register an eager workflow, you'll be able to see it in the task view:
#
# :::{figure} https://github.com/nebulaclouds/static-resources/blob/main/nebulasnacks/user_guide/nebula_eager_workflow_ui_view.png?raw=true
...
# this is because eager workflows are actually Nebula tasks under the hood and
# Nebula has no way of knowing the shape of the execution graph before actually
# executing them.
#
# :::{figure} https://github.com/nebulaclouds/static-resources/blob/main/nebulasnacks/user_guide/nebula_eager_workflow_execution.png?raw=true
# :alt: Eager Workflow Execution
# :class: with-shadow
# :::
#
# However, at the end of execution, you'll be able to use {ref}`Nebula Decks <deck>`
# to see a list of all the tasks and subworkflows that were executed within the
# eager workflow:
#
# :::{figure} https://github.com/nebulaclouds/static-resources/blob/main/nebulasnacks/user_guide/nebula_eager_workflow_deck.png?raw=true
...
# - All exceptions raised by Nebula tasks or workflows will be caught and raised
#   as an {py:class}`~nebulakit.experimental.EagerException` at runtime.
# - All task/subworkflow outputs are materialized as Python values, which includes
#   offloaded types like `NebulaFile`, `NebulaDirectory`, `StructuredDataset`, and
...
#   in eager workflows.
# - Nebula entities that are invoked inside of an eager workflow must be registered
...
# - Nebula console currently does not have a first-class way of viewing eager
#   workflows, but it can be accessed via the task list view and the execution
#   graph is viewable via Nebula Decks.
...
# will help you to reason about the different workflow constructs in Nebula in terms
# of promises and materialized values:
#
# | Construct | Description | Nebula Promises | Pro | Con |
# |--------|--------|--------|----|----|
# | `@workflow` | Compiled at compile-time | All inputs and intermediary outputs are promises | Type errors caught at compile-time | Constrained by Nebula DSL |
# | `@dynamic` | Compiled at run-time | Inputs are materialized, but outputs of all Nebula entities are Promises | More flexible than `@workflow`, e.g. can do Python operations on inputs | Can't use a lot of Python constructs (e.g. try/except) |
