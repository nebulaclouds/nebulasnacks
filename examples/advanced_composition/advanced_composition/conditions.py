...
# ```
#
# Nebulakit supports conditions as a first class construct in the language. Conditions offer a way to selectively execute
...
from nebulakit import conditional, task, workflow
...
#
# In this example, we define an `if` condition with multiple branches. It fails if none of the conditions is met. Nebula
# expects any `conditional()` statement to be **complete**. This means all possible branches should be handled.
#
# :::{note}
# Notice the use of bitwise (&). Python (PEP-335) doesn't allow overloading of the logical `and`, `or`, and `not` operators. Nebulakit uses bitwise `&` and `|` as logical `and` and `or` operators. This is a common practice in other libraries too.
# :::
# %%
@workflow
...
# In a workflow, no output can be accessed directly. The inputs and outputs are auto-wrapped in a special object called {py:class}`nebulakit.extend.Promise`.
# :::
#
...
# the workflow context and is wrapped in a Nebulakit special object. This special object allows it to have the additional
# behavior.
# :::
# %%
@workflow
