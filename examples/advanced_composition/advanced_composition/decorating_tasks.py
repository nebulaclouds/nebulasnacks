...
# information that Nebula needs, you will need to use the built-in {py:func}`~functools.wraps` decorator.
#
...
# {py:func}`~functools.wraps` decorator pattern. To learn more about how to extend Nebula at a deeper level, for
# example creating custom types, custom tasks or backend plugins,
# see {ref}`Extending Nebula <plugins_extend>`.
