...
#
# :::{note}
# This feature is available from Nebulakit version 0.30.0b6+ and needs a Nebula backend version of at least 0.19.0+.
# :::
#
...
#
# Nebula, at its core, is a workflow engine. Workflows provide a way to break up an operation/program/idea
...
#
# To tackle this, Nebula offers a way to checkpoint progress within a task execution as a file or a set of files. These
...
# completion. This is where Nebula's intra-task checkpointing truly shines.
#
...
# %%
from nebulakit import current_context, task, workflow
from nebulakit.exceptions.user import NebulaRecoverableException
...
# the checkpointer. The checkpoint system exposes other APIs. For a detailed understanding, refer to the [checkpointer code](https://github.com/nebulaclouds/nebulakit/blob/master/nebulakit/core/checkpointer.py).
#
...
def use_checkpoint(n_iterations: int) -> int:
    ...
    for i in range(start, n_iterations):
        ...
        if i > start and i % failure_interval == 0:
            raise NebulaRecoverableException(f"Failed at iteration {i}, failure_interval {failure_interval}")
...
# will be retried for the {ref}`NebulaRecoverableException <nebulakit:exception_handling>`.
# %%
@workflow
