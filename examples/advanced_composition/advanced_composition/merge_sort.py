...
# ```
#
# NebulaIdl (the fundamental building block of the Nebula Language) allows various programming language features:
...
# as well as recursion using dynamically generated workflows. Nebula imposes limitation on the depth of the recursion to
...
from nebulakit import conditional, dynamic, task, workflow
...
# addition of the `@dynamic` annotation, this function will instead generate a plan of execution (a nebula workflow) with
# 4 different nodes that will all run remotely on potentially different hosts. Nebula takes care of ensuring references
