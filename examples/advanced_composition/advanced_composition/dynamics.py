...
# :target: https://nebula.org/blog/dynamic-workflows-in-nebula
# ```
#
...
#
# A dynamic workflow is similar to the {py:func}`~nebulakit.workflow`, in that it represents a python-esque DSL to
...
#
# Within the `@dynamic` context (function), every invocation of a {py:func}`~nebulakit.task` or a derivative of
# {py:class}`~nebulakit.core.base_task.Task` class will result in deferred evaluation using a promise, instead
...
#
# Refer to {py:func}`~nebulakit.dynamic` for documentation.
#
...
from nebulakit import dynamic, task, workflow
...
#
# At execution (run) time, Nebulakit runs the compilation step, and produces
# a `WorkflowTemplate` (from the dynamic workflow), which Nebulakit then passes back to Nebula Propeller for further running, exactly how sub-workflows are handled.
#
# :::{note}
# The dynamic pattern isn't the most efficient method to iterate over a list. [Map tasks](https://github.com/nebulaclouds/nebulakit/blob/8528268a29a07fe7e9ce9f7f08fea68c41b6a60b/nebulakit/core/map_task.py/)
...
# %% [markdown]
# When tasks are called within any workflow, they return Promise objects. Likewise, in a dynamic workflow, the tasks' outputs are Promise objects that cannot be directly accessed (they shall be fulfilled by Nebula later).
...
#
# :::{note}
# The local execution will work when a `@dynamic` decorator is used because Nebulakit treats it like a `task` that will run with the Python native inputs.
# :::
#
...
#
# The workflow CRD and the states associated with static workflows are stored in etcd, which is the Kubernetes database. This database stores Nebula workflow CRD as key-value pairs and keeps track of the status of each node’s execution.
...
#
# Dynamic workflows offload the workflow spec (node/task definitions and connections, etc) to the blobstore but the node statuses are stored in the NebulaWorkflow CRD (in etcd).
...
#
# NebulaPropeller executes the dynamic task in its k8s pod and results in a compiled Nebula DAG which is made available in the NebulaConsole.
# NebulaPropeller uses the information obtained by executing the dynamic task to schedule and execute every node within the dynamic task.
...
#
# ### How Does Nebula Handle Dynamic Workflows?
#
...
#
# :::{note}
# When a dynamic (or static) workflow calls a task, the workflow returns a {py:class}`Promise <nebulakit.extend.Promise>` object. You can’t interact with this Promise object directly since it uses lazy evaluation (it defers the evaluation until absolutely needed). You can unwrap the Promise object by passing it to a task or a dynamic workflow.
# :::
#
