...
#
# In this tutorial, we will understand how data-parallel distributed training works with Nebula, Horovod, and Spark.
#
...
from nebulakitplugins.spark import Spark
...
#
# We define a task to download the data into a `NebulaDirectory`.
# %%
@task(
    cache=True,
    cache_version="0.1",
)
def download_data(dataset: str) -> NebulaDirectory:
    ...
    return NebulaDirectory(path=str(data_dir))
...
def data_preparation(
    data_dir: NebulaDirectory, hp: Hyperparameters
    ...
    spark = nebulakit.current_context().spark_session
...
def train(
    ...
    hp: Hyperparameters,
    work_dir: NebulaDirectory,
    train_df: pyspark.sql.DataFrame,
    working_dir: NebulaDirectory,
):
    print("==============")
...
#
# Nebula provides an easy-to-use interface to specify Spark-related attributes.
# The Spark attributes need to be attached to a specific task, and just like that, Nebula can run Spark jobs natively on Kubernetes clusters!
# Within the task, let's call the data pre-processing, training, and evaluation functions.
#
# :::{note}
# To set up Spark, refer to {ref}`nebula-and-spark`.
# :::
# %%
@task(
    task_config=Spark(
...
def horovod_spark_task(data_dir: NebulaDirectory, hp: Hyperparameters, work_dir: NebulaDirectory) -> NebulaDirectory:
...
def horovod_spark_wf(
    ...
    work_dir: NebulaDirectory = "s3://nebula-demo/horovod-tmp/",
) -> NebulaDirectory:
