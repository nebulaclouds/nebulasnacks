...
# ## What is an Agent?
#
# In Nebula, an Agent is a long-running stateless service that can be used to execute tasks. It reduces the overhead of creating a pod for each task.
...
# either run the task in the pod, or you can create a new agent to run it. You can determine how the task will be executed in the NebulaPropeller configMap.
#
...
# - Nebula Agent usage in local development: Users, especially in nebulakit and unionml, can leverage backend agents for local development, streamlining the development process.
...
# creating a CRD and submitting a http request to the external service. However, it increases the complexity of nebulapropeller, and
# it's hard to maintain the backend plugin. For example, if we want to add a new plugin, we need to update and compile
# nebulapropeller, and it's also hard to test. In addition, the backend plugin is running in nebulapropeller itself, so it
# increases the load of the nebulapropeller engine.
#
...
# To address these issues, we introduced the "Agent Service" in Nebula. This system enables rapid plugin
# development while decoupling them from the core nebulapropeller engine.
#
# ## Overview
#
# The Nebula Agent Service is a Python-based agent registry powered by a gRPC server. It allows users and nebulapropeller
...
#
# ### Nebulakit interface specification
#
# To register a new agent, you can extend the `AgentBase` class in the nebulakit backend module. Implementing the following three methods is necessary, and it's important to ensure that all calls are idempotent:
#
...
#
# ```python
# from nebulakit.extend.backend.base_agent import AgentBase, AgentRegistry
...
# ```
#
# Here is an example of [BigQuery Agent](https://github.com/nebulaclouds/nebulakit/blob/9977aac26242ebbede8e00d476c2fbc59ac5487a/plugins/nebulakit-bigquery/nebulakitplugins/bigquery/agent.py#L35) implementation.
#
# ### How to test the agent
#
# Agent can be tested locally without running backend server. It makes the development of the agent easier.
#
# The task inherited from AsyncAgentExecutorMixin can be executed locally, allowing nebulakit to mimic the propeller's behavior to call the agent.
...
# After setting up the CREDENTIALS, you can run the task locally. Nebulakit will automatically call the agent to create, get, or delete the task.
#
# ```python
...
#     task_config=BigQueryConfig(ProjectID="nebula-test-340607")
# )
# ```
#
...
#
# ```bash
# pynebula run wf.py bigquery_doge_coin --version 10
# ```
#
# ### Build a New image
#
# The following is a sample Dockerfile for building an image for a nebula agent.
#
# ```Dockerfile
...
# RUN pip install nebulakitplugins-bigquery
...
# ### Update NebulaAgent
#
# 1. Update the NebulaAgent deployment's [image](https://github.com/nebulaclouds/nebula/blob/c049865cba017ad826405c7145cd3eccbc553232/charts/nebulaagent/templates/agent/deployment.yaml#L26)
# 2. Update the NebulaPropeller configmap.
#
# ```YAML
# tasks:
#   task-plugins:
...
#     agents:
#       custom_agent:
#         endpoint: "dns:///custom-nebulaagent.nebula.svc.cluster.local:8000"
#         insecure: false
