...
#
# This example creates six Nebula tasks that:
#
...
# 6. Reduce dimensions using t-SNE and generate a plot using NebulaDeck.
...
model_file = typing.NamedTuple("ModelFile", model=NebulaFile[MODELSER_NLP])
...
# %% [markdown]
# We declare `NamedTuple`s which will be used as signatures of the Nebula task outputs.
# The variable names and types correspond to the values of the unpacked tuples returned
# from the corresponding Nebula task.
# %%
...
# %% [markdown]
# We define a Nebula task to generate the processed corpus containing a list of tokenized sentence lists.
# %%
@task
...
def train_word2vec_model(training_data: List[List[str]], hyperparams: Word2VecModelHyperparams) -> model_file:
    ...
    out_path = os.path.join(nebulakit.current_context().working_directory, "word2vec.model")
    model.save(out_path)
    return (out_path,)
...
def word_similarities(model_ser: NebulaFile[MODELSER_NLP], word: str) -> Dict[str, float]:
...
# %%
@task(cache_version="1.0", cache=True, limits=Resources(mem="600Mi"))
def word_movers_distance(model_ser: NebulaFile[MODELSER_NLP]) -> float:
...
def dimensionality_reduction(model_ser: NebulaFile[MODELSER_NLP]) -> plotdata:
    model = Word2Vec.load(model_ser.download())
    num_dimensions = 2
...
def plot_with_plotly(x: List[float], y: List[float], labels: List[str]):
    ...
    logger.info("Generating the Word Embedding Plot using Nebula Deck")
    nebulakit.Deck("Word Embeddings", io.to_html(fig, full_html=True))
