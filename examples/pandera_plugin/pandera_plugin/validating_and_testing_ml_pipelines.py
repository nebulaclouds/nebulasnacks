...
import nebulakitplugins.pandera  # noqa: F401
import joblib
import pandas as pd
...
# %% [markdown]
# We also need to import the `pandera` nebulakit plugin to enable dataframe runtime type-checking:
...
#
# Now we're ready to write our first Nebula task:
...
# automatically handled by Nebula so that the pointer to the actual serialized file can be passed onto the last
...
# %% [markdown]
# Finally, we put all of the pieces together in a Nebula workflow:
...
#    model-training pipeline.
# 2. Type-annotate `nebulakit` tasks to automatically type-check dataframes at runtime using the `nebulakitplugins.pandera` plugin.
