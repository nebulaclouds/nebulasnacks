...
# # Pod Example
#
# Pod configuration for a Nebula task allows you to run multiple containers within a single task.
...
#
# The primary container is the main driver for Nebula task execution and is responsible for producing inputs and outputs.
#
...
from nebulakit import Resources, TaskMetadata, dynamic, map_task, task, workflow
from nebulakitplugins.pod import Pod
...
# The `containers` field is set to an empty list, signaling to Nebula to insert a placeholder primary container.
...
# %% [markdown]
# :::{note}
# To configure default settings for all pods Nebula creates, including tasks for pods, containers, PyTorch, Spark, Ray, and Dask,
# [configure a default Kubernetes pod template](https://docs.nebula.org/en/latest/deployment/cluster_config/general.html#using-default-k8s-podtemplates).
# :::
...
# While pod tasks generally allow you to customize Kubernetes container attributes, you can still use Nebula directives to specify resources and the image.
# Unless you specify the `container_image` task attribute, the default image built for Nebula tasks will be used.
# %%
@task(
    task_config=Pod(
        pod_spec=V1PodSpec(
