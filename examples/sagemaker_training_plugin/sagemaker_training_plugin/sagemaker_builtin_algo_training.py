...
# %% [markdown]
# Finally, the Nebulakit plugin called SdkBuiltinAlgorithmTrainingJobTask will be used to create a task that wraps the algorithm.
# This task does not have a user-defined function as the actual algorithm is pre-defined in Sagemaker, but still has the same set of properties like any other NebulaTask:
...
#
# Project (nebulaexamples): the project under which the execution will be created
#
...
# Nebula makes it extremely effective to optimize a model using Amazon Sagemaker HPO. This example will show how
