...
# ```
#
# Let's write a Nebula {py:func}`~nebulakit.workflow` that invokes a
# {py:func}`~nebulakit.task` to generate the output "Hello, World!".
#
...
#
# To begin, import `task` and `workflow` from the `nebulakit` library.
# %%
...
# Store the output in a variable and use it as a return value for a Nebula workflow.
# %%
@workflow
