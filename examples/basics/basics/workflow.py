...
# Registration involves uploading the packaged (serialized) code to the Nebula backend,
# enabling the workflow to be triggered.
#
# For more information, see the {std:ref}`registration documentation <nebula:divedeep-registration>`.
#
# To begin, import {py:func}`~nebulakit.task` and {py:func}`~nebulakit.workflow` from the nebulakit library.
# %%
...
# %% [markdown]
# The {py:func}`~nebulakit.workflow` decorator encapsulates Nebula tasks,
# essentially representing lazily evaluated promises.
# During parsing, function calls are deferred until execution time.
# These function calls generate {py:class}`~nebulakit.extend.Promise`s that can be propagated to downstream functions,
...
# [`pynebula`](https://docs.nebula.org/projects/nebulakit/en/latest/pynebula.html),
# [`nebulactl`](https://docs.nebula.org/projects/nebulactl/en/latest/index.html) or UI, triggering evaluation.
...
# tasks, workflows and other Nebula constructs.
#
# :::{note}
# You can learn more about creating dynamic Nebula workflows by referring
...
# %% [markdown]
# To run the workflow locally, you can use the following `pynebula run` command:
# ```
# pynebula run \
