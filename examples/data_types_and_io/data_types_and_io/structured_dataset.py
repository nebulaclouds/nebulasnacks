...
# ```
# ```{currentmodule} nebulakit.types.structured
# ```
#
...
#
# This example demonstrates how to work with a structured dataset using Nebula entities.
#
...
from nebulakit import NebulaContext, StructuredDatasetType, kwtypes, task, workflow
...
# Nebulakit will detect the Pandas dataframe return signature and
...
# When you invoke `all()` with ``pandas.DataFrame``, the Nebula engine downloads the parquet file on S3, and deserializes it to `pandas.DataFrame`.
...
# %%
from nebulakit.types.structured import register_csv_handlers
from nebulakit.types.structured.structured_dataset import CSV
...
# %% [markdown]
# ## Storage driver and location
# By default, the data will be written to the same place that all other pointer-types (NebulaFile, NebulaDirectory, etc.) are written to.
# This is controlled by the output data prefix option in Nebula which is configurable on multiple levels.
#
# That is to say, in the simple default case, Nebulakit will,
#
...
#
# Two things need to happen with any dataframe instance when interacting with Nebula:
#
...
# Nebulakit decides which of the loaded plugins to invoke based on three attributes:
#
# - The byte format
# - The storage location
# - The Python type in the task or workflow signature.
#
# These three keys uniquely identify which encoder (used when converting a dataframe in Python memory to a Nebula value,
# e.g. when a task finishes and returns a dataframe) or decoder (used when hydrating a dataframe in memory from a Nebula value,
...
# Therefore, Nebulakit has a default byte-format for every Python dataframe type registered with nebulakit.
#
# ## The `uri` argument
#
...
# This is because the stock BigQuery encoder is loaded into Nebulakit with an empty format.
# The Nebulakit `StructuredDatasetTransformerEngine` interprets that to mean that it is a generic encoder
...
#
# :::{note}
# Nebula creates a table inside the dataset in the project upon BigQuery query execution.
# :::
#
...
#    ...
#    return StructuredDataset(df1, uri="bq://project:nebula.table"), \
#           StructuredDataset(df2, uri="gs://auxiliary-bucket/data")
# ```
#
# If you want to customize the Nebula interaction behavior, you'll need to wrap your dataframe in a `StructuredDataset` wrapper object.
#
...
# Python value to a Nebula literal and vice-versa, respectively.
...
class NumpyDecodingHandler(StructuredDatasetDecoder):
    def decode(
        ...
        local_dir = ctx.file_access.get_random_local_directory()
        ctx.file_access.get_data(nebula_value.uri, local_dir, is_multipart=True)
...
# ### NumPy renderer
#
# Create a default renderer for numpy array, then Nebulakit will use this renderer to
# display schema of NumPy array on the Nebula deck.
# %%
class NumpyRenderer:
