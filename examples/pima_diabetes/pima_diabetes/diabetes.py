...
def split_traintest_dataset(
    dataset: NebulaFile[typing.TypeVar("csv")], seed: int, test_split_ratio: float
) -> Tuple[
    NebulaSchema[FEATURE_COLUMNS],
    NebulaSchema[FEATURE_COLUMNS],
    NebulaSchema[CLASSES_COLUMNS],
    NebulaSchema[CLASSES_COLUMNS],
]:
    """
...
# %% [markdown]
# It is also possible in Nebula to pass custom objects, as long as they are
...
model_file = typing.NamedTuple("Model", model=NebulaFile[MODELSER_JOBLIB])
workflow_outputs = typing.NamedTuple("WorkflowOutputs", model=NebulaFile[MODELSER_JOBLIB], accuracy=float)


@task(cache_version="1.0", cache=True, limits=Resources(mem="200Mi"))
def fit(
    x: NebulaSchema[FEATURE_COLUMNS],
    y: NebulaSchema[CLASSES_COLUMNS],
...
def predict(
    x: NebulaSchema[FEATURE_COLUMNS],
    model_ser: NebulaFile[MODELSER_JOBLIB],
) -> NebulaSchema[CLASSES_COLUMNS]:
    """
...
@task(cache_version="1.0", cache=True, limits=Resources(mem="200Mi"))
def score(predictions: NebulaSchema[CLASSES_COLUMNS], y: NebulaSchema[CLASSES_COLUMNS]) -> float:
    """
