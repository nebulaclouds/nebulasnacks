...
from nebulakit.core.base_task import IgnoreOutputs
from nebulakit.types.directory import NebulaDirectory
from nebulakitplugins.kfmpi import Launcher, MPIJob, Worker
...
# %% [markdown]
# To create an MPI task, add {py:class}`~nebulakitplugins.kfmpi.MPIJob` config to the Nebula task.
...
# [`HorovodJob`](https://github.com/nebulaclouds/nebulakit/blob/master/plugins/nebulakit-kf-mpi/nebulakitplugins/kfmpi/task.py#L222)
...
def horovod_train_task(batch_size: int, buffer_size: int, dataset_size: int) -> NebulaDirectory:
    import horovod.tensorflow as hvd
    ...
    return NebulaDirectory(path=str(working_dir))
...
# %%
@workflow
def horovod_training_wf(batch_size: int = 128, buffer_size: int = 10000, dataset_size: int = 10000) -> NebulaDirectory:
    """
