...
from nebulakit.types.file import JPEGImageFile, ONNXFile
from nebulakitplugins.onnxpytorch import PyTorch2ONNX, PyTorch2ONNXConfig
...
# This is a special annotation that tells Nebulakit that this parameter is to be converted to an ONNX model with the given config.
# %%
@task
def train() -> Annotated[
    PyTorch2ONNX,
...
def onnx_predict(model_file: ONNXFile) -> JPEGImageFile:
    ...
    img = Image.open(
        requests.get(
            "https://raw.githubusercontent.com/nebulaclouds/static-resources/main/nebulakit/onnx/cat.jpg",
            stream=True,
        ).raw
    )
    ...
    img_path = Path(nebulakit.current_context().working_directory) / "cat_superres_with_ort.jpg"
