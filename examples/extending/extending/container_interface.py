# %% [markdown]
# (core-extend-nebula-container-interface)=
#
# # Container Interface
#
# ```{eval-rst}
...
# ```
#
# Nebula typically interacts with containers in the course of its task execution (since most tasks are container
...
#
#    1. This is done by nebulakit for instance for your run of the mill `@task`. This step is **crucial** - the task needs to specify an image available in the registry configured in the nebula installation.
#
# 2. At runtime, Nebula will execute your task via a plugin. The default container plugin will do the following:
#
...
# ## Command Templating
#
# The templating of container arguments at run-time is one of the more advanced constructs of Nebula, but one that
...
# | {{.Inputs.\<xyz>}}       | `"hello world"`            | For primitive inputs, the task can request that Nebula unpack the actual literal value, saving the task from having to download the file. Note that for Blob, Schema and StructuredDataset types, the uri where the data is stored will be filled in as the value. |
