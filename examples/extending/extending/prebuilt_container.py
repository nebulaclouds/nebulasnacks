...
# - Makes it possible to (largely) extend the Nebula task execution behavior without using the backend GOlang plugin.
...
# ## Usage
#
# Take a look at the [example PR](https://github.com/nebulaclouds/nebulakit/pull/470), where we switched the built-in SQLite3 task from the old (user-container) to the new style of writing tasks.
#
# There aren't many changes from the user's standpoint:
# \- Install whichever Python library has the task type definition (in the case of SQLite3, it's bundled in Nebulakit, but this isn't always the case (for example, [SQLAlchemy](https://github.com/nebulaclouds/nebulakit/tree/master/plugins/nebulakit-sqlalchemy))).
...
# 3. Creating a Dockerfile that is executed when any user runs your task. It'll most likely include Nebulakit, Python, and your task extension code.
#
# To follow along, use the [PR (mentioned above)](https://github.com/nebulaclouds/nebulakit/pull/470) where we migrate the SQLite3 task.
#
# ## Python Library
#
# ### Defining a task
...
#
# - `container_image`: This is the container image that will run on a Nebula platform when the user invokes the job.
# - `executor_type`: This should be the Python class that inherits the `ShimTaskExecutor`.
# - `task_type`: All types have a task type. Nebula engine uses this string to identify which plugin to use when running a task.
#
...
#
# ```
# container_image="ghcr.io/nebulaclouds/nebulakit:py38-v0.19.0b7",
# executor_type=SQLite3TaskExecutor,
# task_type="sqlite",
# ```
#
# Note that the container is special in this case since we utilize the Nebulakit image.
#
...
# This function stores and inserts the data into the template's [custom field](https://github.com/nebulaclouds/nebulaidl/blob/7302971c064b6061a148f2bee79f673bc8cf30ee/protos/nebulaidl/core/tasks.proto#L114).
...
# ### Image
#
# This is the custom image that you specified in the subclass `PythonCustomizedContainerTask`. Out of the box, when Nebula runs the container, these tasks will run a command that looks like this
#
# ```
# pynebula-execute --inputs s3://inputs.pb --output-prefix s3://outputs --raw-output-data-prefix s3://user-data --resolver nebulakit.core.python_customized_container_task.default_task_template_resolver -- {{.taskTemplatePath}} path.to.your.executor.subclass
# ```
#
# This means that your [Docker image](https://github.com/nebulaclouds/nebulakit/blob/master/Dockerfile) will need Python and Nebulakit installed.
