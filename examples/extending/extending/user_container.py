...
# in the object store. Remember that if you follow the nebula/nebulakit constructs, you will automatically make your plugin portable
# across all cloud platforms that Nebula supports.
#
# ## Sensor Plugin
#
# A sensor plugin waits for some event to happen before marking the task as success. You need not worry about the
# timeout as that will be handled by the nebula engine itself when running in production.
#
# ### Plugin API
#
# ```python
...
from nebulakit import TaskMetadata, task, workflow
from nebulakit.extend import Interface, PythonTask, context_manager
...
# derives from {py:class}`nebulakit.PythonFunctionTask` as follows.
# %%
...
# #### Config Objects
#
# Nebulakit routes to the right plugin based on the type of `task_config` class if using the `@task` decorator.
...
# :::
#
# Refer to the [spark plugin](https://github.com/nebulaclouds/nebulakit/tree/master/plugins/nebulakit-spark) for an example of a config object.
