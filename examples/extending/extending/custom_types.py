...
#
# # Writing Custom Nebula Types
#
# ```{eval-rst}
# .. tags:: Extensibility, Contribute, Intermediate
# ```
#
# Nebula is a strongly-typed framework for authoring tasks and workflows. But there are situations when the existing
...
# Nebulakit allows modeling user classes. The idea is to make an interface that is more productive for the
# use case, while writing a transformer that converts the user-defined type into one of the generic constructs in Nebula's type system.
#
# This example will try to model an example user-defined dataset and show how it can be seamlessly integrated with Nebulakit's type engine.
#
...
from nebulakit import Blob, BlobMetadata, BlobType, NebulaContext, Literal, LiteralType, Scalar, task, workflow
from nebulakit.extend import TypeEngine, TypeTransformer


# %% [markdown]
# :::{note}
# `NebulaContext` is used to access a random local directory.
# :::
#
...
class MyDataset(object):
    """
    ``MyDataset`` is a collection of files. In Nebula, this maps to a multi-part blob or directory.
    """
...
# Nebula's remote references. To do this, we create a new instance of
# {py:class}`~nebulakit:nebulakit.extend.TypeTransformer`, for the type `MyDataset` as follows:
#
# :::{note}
...
class MyDatasetTransformer(TypeTransformer[MyDataset]):
    ...
    def get_literal_type(self, t: Type[MyDataset]) -> LiteralType:
        """
        This is useful to tell the Nebulakit type system that ``MyDataset`` actually refers to what corresponding type.
    ...
    def to_literal(
        ...
        """
        # Step 1: Upload all the data into a remote place recommended by Nebula
    ...
    def to_python_value(self, ctx: NebulaContext, lv: Literal, expected_python_type: Type[MyDataset]) -> MyDataset:
        """
        In this method, we want to be able to re-hydrate the custom object from Nebula Literal value.
        """
...
# %% [markdown]
# Before we can use MyDataset in our tasks, we need to let Nebulakit know that `MyDataset` should be considered as a valid type.
# This is done using {py:class}`~nebulakit:nebulakit.extend.TypeEngine`'s `register` method.
# %%
...
# %% [markdown]
# This workflow can be executed and tested locally. Nebulakit will exercise the entire path even if you run it locally.
#
# %%
