...
# # dbt example
#
# In this example we're going to execute dbt commands supported by Nebula: `dbt run`,
# `dbt test`, and `dbt source freshness`.
#
# First, let's import what we need from `nebulakit` and `nebulakitplugins-dbt`.
# %%
import subprocess
from typing import Tuple

from nebulakit import task, workflow
from nebulakitplugins.dbt.schema import (
    DBTFreshnessInput,
    DBTFreshnessOutput,
    DBTRunInput,
    DBTRunOutput,
...
from nebulakitplugins.dbt.task import DBTFreshness, DBTRun, DBTTest
...
def prepare_and_seed_database():
    ...
    subprocess.run(
        [
            "psql",
            "-h",
            "sandbox-postgresql.nebula.svc.cluster.local",
            "-p",
            "5432",
            "-U",
            "postgres",
