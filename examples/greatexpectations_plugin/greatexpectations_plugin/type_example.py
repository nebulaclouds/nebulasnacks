...
# In this example, we'll implement a simple task, followed by Great Expectations data validation on `NebulaFile`, `NebulaSchema`, and finally,
...
from nebulakit.types.file import CSVFile
from nebulakit.types.schema import NebulaSchema
from nebulakitplugins.great_expectations import BatchRequestConfig, GreatExpectationsNebulaConfig, GreatExpectationsType
...
# Moreover, there's `GreatExpectationsNebulaConfig` that encapsulates the essential initialization parameters of the plugin.
# :::
...
# ## NebulaFile
#
# First, we define `GreatExpectationsNebulaConfig` to initialize all our parameters. Here, we're validating a `NebulaFile`.
# %%
great_expectations_config = GreatExpectationsNebulaConfig(
    datasource_name="data",
    expectation_suite_name="test.demo",
    data_connector_name="data_nebulatype_data_connector",
    local_file_path="/tmp",
    context_root_dir=CONTEXT_ROOT_DIR,
)
...
# Under the hood, `GreatExpectationsType` validates data in accordance with the `GreatExpectationsNebulaConfig` defined previously.
# This `GreatExpectationsNebulaConfig` is being fetched under the name `great_expectations_config`.
#
# The first value that's being sent within `GreatExpectationsType` is `CSVFile` (this is a pre-formatted NebulaFile type).
# This means that we want to validate the `NebulaFile` data.
...
# ## NebulaSchema
#
# We define a `GreatExpectationsType` to validate `NebulaSchema`. The `local_file_path` is where we would have our parquet file.
#
# :::{note}
...
def schema_task(
    dataframe: GreatExpectationsType[
        NebulaSchema,  # noqa: F821
        GreatExpectationsNebulaConfig(  # noqa: F821
            datasource_name="data",  # noqa: F821
            expectation_suite_name="test.demo",  # noqa: F821
            data_connector_name="data_nebulatype_data_connector",  # noqa: F821
...
# %% [markdown]
# Finally, we define a workflow to call our task.
# We're using DataFrame returned by the `file_task` that we defined in the `NebulaFile` section.
# %%
@workflow
def schema_wf() -> int:
...
# :::
# %%
runtime_ge_config = GreatExpectationsNebulaConfig(
...
# %%
@task
def runtime_validation(dataframe: GreatExpectationsType[NebulaSchema, runtime_ge_config]) -> int:
...
if __name__ == "__main__":
    ...
    print(simple_wf())
    print("Great Expectations Type with NebulaFile...")
    print(file_wf())
    print("Great Expectations Type with NebulaSchema...")
    print(schema_wf())
