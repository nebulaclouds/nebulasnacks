...
#
# It shows how distributed training can be completely performed on the user side with minimal changes using Nebula.
#
# :::{NOTE}
# Nebulakit will be adding further simplifications to make writing a distributed training algorithm even simpler, but
...
def mnist_pytorch_job(hp: Hyperparameters) -> PythonPickledFile:
    ...
    # These can be replaced by Nebula's blob type inputs. Note that the data here are assumed
