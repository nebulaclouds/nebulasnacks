...
#
# In this tutorial, we will understand how to predict house prices in multiple regions using XGBoost, and {ref}`dynamic workflows <dynamic_workflows>` in Nebula.
#
# We will split the generated dataset into train, test and validation set.
#
# Next, we will create two dynamic workflows in Nebula, that will:
#
...
import pandas as pd
from nebulakit import Resources, dynamic, workflow
...
# %% [markdown]
# Next, we create a {py:func}`~nebulakit:nebulakit.dynamic` workflow to generate and split the data for multiple regions.
...
#
# We create another {py:func}`~nebulakit:nebulakit.dynamic` workflow to train the model and generate predictions.
