...
import pandas as pd
import requests
from nebulakit import conditional, kwtypes, task, workflow
from nebulakit.extras.tasks.shell import OutputLocation, ShellTask
from nebulakit.types.file import NebulaFile, PNGImageFile
...
def download_dataset():
    ...
    r = requests.get("https://api.github.com/repos/nebulaclouds/nebulasnacks/contents/blast/kitasatospora?ref=datasets")
...
blastx_on_shell = ShellTask(
    ...
    output_locs=[
        OutputLocation(var="stdout", var_type=NebulaFile, location="stdout.txt"),
        OutputLocation(
            var="blastout",
            var_type=NebulaFile,
...
@task
def blastx_output(blastout: NebulaFile) -> BLASTXOutput:
    # read BLASTX output
...
# %%
@task
def is_batchx_success(stdout: NebulaFile) -> bool:
    if open(stdout).read():
        return False
    else:
        return True
