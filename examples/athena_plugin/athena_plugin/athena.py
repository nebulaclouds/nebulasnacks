...
# # Athena Query
#
# This example shows how to use a Nebula AthenaTask to execute a query.
...
# Athena task a name that's unique across your project/domain for your Nebula installation.
# %%
...
# We'll produce a NebulaSchema that we can use in downstream nebula tasks for further analysis or manipulation.
...
athena_task_templatized_query = AthenaTask(
    ...
    """,
    # While we define a generic schema as the output here, Nebula supports more strongly typed schemas to provide
    # better compile-time checks for task compatibility. Refer to {py:class}`nebulakit.NebulaSchema` for more details
...
# %% [markdown]
# Now we (trivially) clean up and interact with the data produced from the above Athena query in a separate Nebula task.
#
# %%
@task
def manipulate_athena_schema(s: NebulaSchema) -> NebulaSchema:
    df = s.open().all()
    return df


@workflow
def full_athena_wf(country_iso_code: str) -> NebulaSchema:
