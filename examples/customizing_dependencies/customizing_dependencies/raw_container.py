...
# ```
#
# This example demonstrates how to use arbitrary containers in 5 different languages, all orchestrated in nebulakit seamlessly.
# Nebula mounts an input data volume where all the data needed by the container is available, and an output data volume
...
from nebulakit import ContainerTask, kwtypes, task, workflow
...
# ## Container Tasks
#
# A {py:class}`nebulakit.ContainerTask` denotes an arbitrary container. In the following example, the name of the task
...
# - `output_data_dir` -> where Nebula will expect the outputs to exist.
#
...
calculate_ellipse_area_shell = ContainerTask(
    ...
    image="ghcr.io/nebulaclouds/rawcontainers-shell:v2",
    command=[
...
calculate_ellipse_area_python = ContainerTask(
    ...
    inputs=kwtypes(a=float, b=float),
    outputs=kwtypes(area=float, metadata=str),
    image="ghcr.io/nebulaclouds/rawcontainers-python:v2",
    command=[
        "python",
...
calculate_ellipse_area_r = ContainerTask(
    ...
    inputs=kwtypes(a=float, b=float),
    outputs=kwtypes(area=float, metadata=str),
    image="ghcr.io/nebulaclouds/rawcontainers-r:v2",
    command=[
        "Rscript",
        "--vanilla",
...
calculate_ellipse_area_haskell = ContainerTask(
    ...
    inputs=kwtypes(a=float, b=float),
    outputs=kwtypes(area=float, metadata=str),
    image="ghcr.io/nebulaclouds/rawcontainers-haskell:v2",
    command=[
...
calculate_ellipse_area_julia = ContainerTask(
    ...
    inputs=kwtypes(a=float, b=float),
    outputs=kwtypes(area=float, metadata=str),
    image="ghcr.io/nebulaclouds/rawcontainers-julia:v2",
    command=[
        "julia",
...
# correspond to the declared input variables. All data returned by the tasks are consumed and logged by a Nebula task.
# %%
@workflow
...
# %% [markdown]
# One of the benefits of raw container tasks is that Nebulakit does not need to be installed in the target container.
#
# :::{note}
