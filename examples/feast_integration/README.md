...
:target: https://blog.nebula.org/bring-ml-close-to-data-using-feast-and-nebula
...
Nebula provides a way to train models and perform feature engineering as a single pipeline.
...
On leveraging the collective capabilities, Nebula enables engineering the features, and Feast provides the feature registry and online feature serving system.
Moreover, Nebula can help ensure incremental development of features and enables to turn on the sync to online stores only when one is confident about the features.

In this tutorial, we'll walk through how Feast can be used to store and retrieve features to train and test the model through a pipeline curated using Nebula.
...
like Snowflake, Athena, Hive, BigQuery, or Spark, all of which are supported by Nebula.
