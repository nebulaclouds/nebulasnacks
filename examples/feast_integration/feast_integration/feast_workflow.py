# %% [markdown]
# # Nebula Pipeline with Feast
#
# In this example, let's build a Nebula pipeline with Feast as the feature store.
#
...
#
# :::{note}
# Run `nebulactl demo start` before running the workflow locally.
# :::
...
from nebulakit import NebulaContext, Resources, task, workflow
from nebulakit.extras.sqlite3.task import SQLite3Config, SQLite3Task
from nebulakit.types.file import NebulaFile, JoblibSerializedFile
...
if os.getenv("DEMO") is None:
    ...
    os.environ["FEAST_S3_ENDPOINT_URL"] = ENDPOINT = "http://minio.nebula:9000"
...
#
# :::{note}
# The Feast feature store is mutable, so be careful, as Nebula workflows can be highly concurrent!
# TODO: use postgres db as the registry to support concurrent writes.
# :::
# %%
@task(limits=Resources(mem="400Mi"))
def store_offline(repo_config: RepoConfig, dataframe: StructuredDataset) -> NebulaFile:
    horse_colic_entity = Entity(name="Hospital Number")

    ctx = nebulakit.current_context()
    data_dir = os.path.join(ctx.working_directory, "parquet-data")
    os.makedirs(data_dir, exist_ok=True)

    NebulaContext.current_context().file_access.get_data(
    ...
    return NebulaFile(path=repo_config.online_store.path)
...
# %%
@task(limits=Resources(mem="400Mi"))
def store_online(repo_config: RepoConfig, online_store: NebulaFile) -> NebulaFile:
    # download the online store file and copy the content to the actual online store path
    NebulaContext.current_context().file_access.get_data(online_store.download(), repo_config.online_store.path)
...
# %%
@workflow
def featurize(repo_config: RepoConfig, imputation_method: str = "mean") -> (StructuredDataset, NebulaFile):
