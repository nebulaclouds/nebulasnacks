...
Nebula is well-suited for data engineering use cases, where you can interleave
...
{ref}`nebulakitplugins-sqlalchemy <sql_alchemy>` plugin, which provides an
interface to perform SQL queries via the
{py:class}`~nebulakitplugins.sqlalchemy.SQLAlchemyTask`
and {py:class}`~nebulakitplugins.sqlalchemy.SQLAlchemyConfig` classes.
...
{py:class}`~nebulakitplugins.sqlalchemy.SQLAlchemyTask` constructor, then
pass in a `secret_connect_args` argument to the
{py:class}`~nebulakitplugins.sqlalchemy.SQLAlchemyConfig` constructor, assuming
...
task's resource requirements with the {py:class}`~nebulakit.Resources` object.
...
blob storage. Nebula has a built-in `CSVFile` type that automatically handles
...
def load(df: pd.DataFrame) -> CSVFile:
    ...
    csv_file = os.path.join(nebulakit.current_context().working_directory, "rna_df.csv")
...
directory, but when the workflow is run on a Nebula cluster, this file lives in
...
Because Nebula tasks and workflows are simply functions, we can embed
...
```{important}
Prefer other data processing frameworks? Nebula ships with
[Polars](https://github.com/nebulaclouds/nebulakit/tree/master/plugins/nebulakit-polars),
{ref}`Dask <plugins-dask-k8s>`, {ref}`Modin <modin-integration>`, {ref}`Spark <plugins-spark-k8s>`,
[Vaex](https://github.com/nebulaclouds/nebulakit/tree/master/plugins/nebulakit-vaex),
and [DBT](https://github.com/nebulaclouds/nebulakit/tree/master/plugins/nebulakit-dbt)
integrations.

For database connectors, Nebula provides first-party support for {ref}`AWS Athena <aws-athena>`,
