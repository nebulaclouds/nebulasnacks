...
(getting_started_extending_nebula)=

# Extending Nebula

Once you have a hang of the fundamentals of Nebula, you may find that you want
to use it in ways that aren't supported out-of-the-box. Fortunately, Nebula
...
the Nebula type system and Nebula tasks.

(customizing_nebula_types)=

## Customizing Nebula Types

Nebula has a {ref}`rich type system <data_types_and_io>` that automatically
...
However, the types that ship with Nebula or one of Nebula's
...
let you compose several Nebula-supported types into a single object. For
...
that are already supported by Nebula.

To create entirely new types, you'll need to use the
{py:class}`~nebulakit.extend.TypeTransformer` interface to explicitly handle
...
## Customizing Nebula Tasks

The easiest way to extend Nebula tasks is to use Python decorators. Since Nebula
...
by the MLFlow integration via the {py:func}`~nebulakitplugins.mlflow.mlflow_autolog`
...
Nebula also supports a setup-teardown pattern at the workflow level, which
...
many more ways to customize Nebula tasks:
...
* - {ref}`Raw Container Tasks <raw_container>`
  - These tasks can be implemented in other programming languages like R,
    ...
    in other languages outside of the `nebulakit` SDK language.
* - {ref}`Backend Plugins <extend-plugin-nebula-backend>`
...
Congratulations! 🎉 You've just completed the Nebula Fundamentals tour.
