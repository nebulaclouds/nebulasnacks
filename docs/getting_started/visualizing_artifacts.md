...
Nebula {py:class}`~nebulakit.deck.Deck`s are one of the first-class constructs in
Nebula, allowing you to generate static HTML reports associated with any of the
...
Nebula materializes Decks via `Renderer`s, which are specific implementations of
how to generate an HTML report from some Python object.

## Enabling Nebula Decks

To enable Nebula Decks, simply set `disable_deck=False` in the `@task` decorator:
...
By default, Nebula will render the inputs and outputs of tasks with the built-in
renderers in the corresponding **input** and **output** {py:class}`~nebulakit.deck.Deck`s,
...
# Nebula Decks directly in the docs.
import datetime
import logging
import os
import re
...
class DeckFilter(logging.Filter):
    ...
    def filter(self, record):
        patt = "(.+) task creates nebula deck html to (.+/deck.html)"
...
def cp_deck(src):
    src = Path(src)
    target = Path.cwd() / "_nebula_decks" / src.parent.name
    target.mkdir(parents=True, exist_ok=True)
    shutil.copy(src, target)
    return target / "deck.html"


logger = logging.getLogger("nebulakit")
logger.setLevel(20)
...
locally, you need to set the `NEBULA_SDK_LOGGING_LEVEL` environment variable
...
deck, which you can access with the {py:func}`~nebulakit.current_context`
...
- A boxplot of the `sepal_length` variable using {py:class}`~nebulakitplugins.deck.renderer.BoxRenderer`,
  which leverages the `plotly` package to auto-generate a set of plots and
  summary statistics from the dataframe.

```{code-cell} ipython3
import nebulakit
from nebulakitplugins.deck.renderer import MarkdownRenderer, BoxRenderer
...
def iris_data(
    ...
    nebulakit.current_context().default_deck.append(MarkdownRenderer().to_html(md_text))
    nebulakit.Deck("box plot", BoxRenderer("sepal_length").to_html(data))
    return data
```

This will create new tab in the Nebula Deck HTML view named **default**, which
...
What if we don't want to show raw data values in the Nebula Deck? We can create a
...
As you can see above, Nebula renders in-line decks in the order in which they are
...
Nebula Decks are simple to customize, as long as you can render the Python object
into some HTML representation.

Learn more about Nebula Decks in the {ref}`User Guide <decks>`.
```

## What's Next?

In this guide, you learned how to generate static HTML reports to gain more
visibility into Nebula tasks. In the next guide, you'll learn how to optimize
