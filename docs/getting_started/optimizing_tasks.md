...
There are many ways to optimize your tasks and workflows in Nebula, and this guide
...
Caching works both locally and on a Nebula backend.
...
Nebula simply hits the cache to obtain the result.

```{note}
For file-like data types like {py:class}`nebulakit.types.file.NebulaFile` and offloaded
...
Nebula also allows you to automatically retry failing tasks in the case of
...
```{note}
Retries only take effect when running a task on a Nebula cluster.
```
...
If you need to parallelize a task, you can use the {py:func}`~nebulakit.map_task`
...
from nebulakit import map_task
...
As one of the core features of Nebula, workflows can be composed of tasks that
potentially have heterogeneous resource requirements. You can express this with
the {py:class}`~nebulakit.Resources` object:

```{code-cell} ipython3
from nebulakit import Resources
...
GPUs. For the former task, we use the default image that ships with `nebulakit`
and for the latter task, we specify a pre-built image that the core Nebula team
...
@task(
    ...
    container_image="ghcr.io/nebulaclouds/nebulakit:py3.9-latest",
)
...
@task(
    ...
    container_image="ghcr.io/nebulaclouds/nebulacookbook:kfpytorch-latest",
)
def train_model(features: np.ndarray, target: np.ndarray) -> nn.Module:
    ...  # train a model using gpus
```

These tasks assume that we're going to use the {ref}`pynebula register <getting_started_pynebula_register>`
command to register these tasks, since these static images will not contain
the code that we defined above. Using `pynebula register` ensures that `get_data`
and `train_model` are zipped up and Nebula has access to it when they're executed
on a Nebula backend.
...
Finally, staying with the theme of Nebula's ability to handle heterogeneous
...
{py:class}`~nebulakitplugins.kfpytorch.PyTorch` plugin:

```{code-cell} ipython3
from nebulakitplugins.kfpytorch import PyTorch
...
Nebula: the ability to *declaratively specify infrastructure requirements*
at the most granular level of your workflow!

When this task is executed on a Nebula cluster, it automatically provisions all of
the resources that you need. In this case, that need is distributed
training, but Nebula also provides integrations for {ref}`Spark <plugins-spark-k8s>`,
{ref}`Ray <kube-ray-op>`, {ref}`MPI <kf-mpi-op>`, {ref}`Sagemaker <aws-sagemaker>`,
{ref}`Snowflake <plugins-snowflake>`, and more.

Even though Nebula itself is a powerful compute engine and orchestrator for
data engineering, machine learning, and analytics, perhaps you have existing
code that leverages other platforms. Nebula recognizes the pain of migrating code,
...
the Nebula Fundamentals tour, we'll see how to extend Nebula in the cases where
