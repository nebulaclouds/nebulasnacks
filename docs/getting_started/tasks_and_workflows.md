...
In the {ref}`Introduction to Nebula <getting_started_index>`, we got a basic sense
of how Nebula works by creating a few tasks and a simple model-training workflow.
In this guide, you'll learn more about how tasks and workflows fit into the Nebula
programming model.

## Tasks

Nebula tasks are the core building blocks of larger, more complex workflows.

### Tasks are Containerized Blocks of Compute

You can think of a Nebula task as a containerized block of compute. When a task
runs on a Nebula backend, it's isolated within its own container, separate from
...
with {py:func}`@task <nebulakit.task>`. We can run this function just like any
...
- Most of the Nebula tasks you'll ever write can be executed locally.
- Tasks and workflows must be invoked with keyword arguments.
- When a task runs on a Nebula cluster, it runs on a
  [Kubernetes Pod](https://kubernetes.io/docs/concepts/workloads/pods/), where
  Nebula orchestrates what task to run at what time in the context of a workflow.
```

### Tasks are Strongly Typed

You might also notice that the `mean` function signature is type-annotated with
Python type hints. Nebula uses these annotations to check the input and output
types of the task when it's compiled or invoked.

Under the hood, Nebula uses its own type system that translates values to and from
Nebula types and the SDK language types, in this case Python. The Nebula type
...
with more complex data types and pipelines, Nebula's type system becomes
invaluable for catching bugs early.

Nebula's type system is also used for caching, data lineage tracking, and
...
This is where Nebula workflows can help us manage the added complexity.
...
that's decorated with {py:func}`@workflow <nebulakit.workflow>`:
...
Although Nebula workflow syntax looks like Python code, it's actually a
...
- Within workflows, the inputs of workflow and the outputs of tasks function are promises under the hood,
  ...
  _You can only pass promises into tasks, workflows, and other Nebula constructs_.
...
When we decorate `standard_scale_workflow_with_print` with `@workflow`, Nebula
compiles an execution graph that's defined inside the function body, so
_it doesn't actually run the computations yet_. Therefore, when Nebula compiles a
workflow, the outputs of task calls are actually promises and not regular python
values.

### Workflows are Strongly Typed Too

Since both tasks and workflows are strongly typed, Nebula can actually catch
type errors! When we learn more about packaging and registering in the next few
guides, we'll see that Nebula can also catch compile-time errors even before
...
```{important}
Learn more about chaining nebula entities in the {ref}`User Guide <chain_nebula_entities>`.
```

## Launch plans

A Nebula {py:class}`~nebulakit.LaunchPlan` is a partial or complete binding of
...
that the latter will kick off a new workflow execution on the Nebula cluster with
...
Next, we'll see how to organize a Nebula project that follows software
