...
Nebula can handle a full spectrum of machine learning workloads, from
...
`Hyperparameter` Python object that provides more type information to the Nebula
compiler. For example, Nebula uses this type information to auto-generate
type-safe launch forms on the Nebula UI. Learn more in the
{ref}`Extending Nebula <customizing_nebula_types>` guide.
```
...
* - **🏔 Vertical Scaling**
  - Use the {py:class}`~nebulakit.Resources` task keyword argument to request
    additional CPUs, GPUs, and/or memory.
* - **🗺 Horizontal Scaling**
  - With constructs like {py:func}`~nebulakit.dynamic` workflows and
    {py:func}`~nebulakit.map_task`s, implement gridsearch, random search,
    and even [bayesian optimization](https://github.com/nebulaclouds/nebulakit-python-template/tree/main/bayesian-optimization/%7B%7Bcookiecutter.project_name%7D%7D).
* - **🔧 Specialized Tuning Libraries**
  - Use the {ref}`Ray Integration <kube-ray-op>` and leverage tools like
    ...
    optimization, all orchestrated by Nebula as ephemerally-provisioned Ray clusters.
...
* - **🔎 Experiment Tracking**
  - Auto-capture training logs with the {py:func}`~nebulakitplugins.mlflow.mlflow_autolog`
    decorator, which can be viewed as Nebula Decks with `@task(disable_decks=False)`.
